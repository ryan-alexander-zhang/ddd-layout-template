...
public class InboxRepositoryImpl implements InboxRepository {
  ...
  @Override
  public TryStartResult tryStart(String messageId, String consumer, String payloadHash,
      Instant now) {
    int inserted = mapper.tryInsertProcessing(messageId, consumer, InboxStatus.PROCESSING.name(),
        payloadHash, now);
    if (inserted == 1) {
  ...
  public Optional<InboxRecord> find(String messageId, String consumer) {
    ...
    return Optional.of(
        new InboxRecord(messageId, consumer, InboxStatus.valueOf(row.status()), row.retryCount()));
  }
